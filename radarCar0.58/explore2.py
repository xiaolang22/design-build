...
def update_plot(ax_explore, ax_full=None):
    ...
    # 累积可见区域
    if visible_grid_mask is None or not isinstance(visible_grid_mask, np.ndarray):
        visible_grid_mask = np.zeros_like(grid_map, dtype=bool)
...
def save_exploration_results(map_file, history_points_cleaned, start_x, start_y, merged_exits):
    """
    Save the exploration results to both .pkl and .txt files
    
    Parameters:
    map_file: Map file path
    history_points_cleaned: Clean point cloud data
    start_x, start_y: Starting position coordinates
    merged_exits: List of detected exits
    
    Returns:
    explore_result_filename: Path to the saved pkl file
    output_filename: Path to the saved txt file
    """
    # Save point cloud, start point and exit data to .pkl file
    import pickle
    explore_result_filename = map_file.split('.')[0] + '_result.pkl'
    print(f"\nSaving exploration results to file: {explore_result_filename}")
    
    try:
        # Create data dictionary
        explore_data = {
            'point_cloud': history_points_cleaned,  # Point cloud list [(x1, y1), (x2, y2), ...]
            'start': (start_x, start_y),  # Starting point coordinates (x, y)
            'exits': merged_exits  # Exit coordinates list [(x1, y1), (x2, y2), ...]
        }
        ...
        print(f"Successfully saved exploration results to {explore_result_filename}")
        print(f"Data format: Dictionary with keys 'point_cloud', 'start', and 'exits'")
        print(f"- Point cloud data: {len(history_points_cleaned)} points")
        print(f"- Start position: ({start_x}, {start_y})")
        print(f"- Exit positions: {len(merged_exits)}")
        
    except Exception as e:
        print(f"Error saving exploration results: {str(e)}")
    
    # Save radar data to .txt file
    output_filename = map_file.split('.')[0] + '_radar_data.txt'
    print(f"\nSaving radar data to file: {output_filename}")
    
    try:
        from common.utils import RES
        with open(output_filename, 'w') as f:
            # First line with metadata: grid size and car radius
            # Format: METADATA GRID_SIZE 0.05 CAR_RADIUS 0.05
            grid_size = RES  # Grid size imported from common.utils
            car_radius = RES  # Car radius defaults to one grid size
            f.write(f"METADATA GRID_SIZE {grid_size} CAR_RADIUS {car_radius}\n")
            
            # Save radar scan data, one scan per line
            for car_pos, radar_scan in all_radar_data:
                # Replace None values with -1, 50+ values (no wall intersection) with 0
                radar_line = []
                for val in radar_scan:
                    if val is None:
                        radar_line.append('-1')
                    elif val >= 40:  # Values ≥ 40 are treated as no wall intersection
                        radar_line.append('0')
                    else:
                        radar_line.append(str(round(val, 3)))
                
                # Each line starts with car position: CAR_X CAR_Y radar_data...
                car_x, car_y = car_pos
                f.write(f"{car_x} {car_y} " + ' '.join(radar_line) + '\n')
            
            # Second-to-last line with entrance and exit info: ENTRANCE x y EXIT1 x1 y1 EXIT2 x2 y2...
        ...
        print(f"Successfully saved {len(all_radar_data)} radar scans to text file")
        print(f"File format: First line contains metadata (grid size and car radius)")
        print(f"           Middle lines format is 'car_x car_y radar_data(360 values)'")
        print(f"           Last line contains entrance and exit coordinates")
        print(f"Note: Radar values -1 mean invalid data, 0 means no wall intersection (ray value ≥ 40m)")
        print(f"Metadata info: Grid size={grid_size}m, Car radius={car_radius}m")
        
    except Exception as e:
        print(f"Error saving radar data: {str(e)}")
        
    return explore_result_filename, output_filename

def show_exploration_results(history_points_cleaned, start_x, start_y, merged_exits):
    """
    Create a final visualization of the exploration results
    ...
    """
    print(f"\n探索完成！")
    print(f"发现 {len(merged_exits)} 个有效出口")
    print(f"收集到 {len(history_points_cleaned)} 个去重后的点云数据")
    ...
    if history_points_cleaned:
        ...
        print(f"点云密度: {len(history_points_cleaned)} 个点")
    else:
        print("未收集到点云数据")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-planner', action='store_true', help='禁用自动调用map_and_path_planner.py')
    ...
    def on_start(event):
        if not exploring[0]:
            ...
            paused[0] = False
            print("开始探索")
            ax_explore.set_title('正在探索...', color="#122bea")
    ...
    def on_pause(event):
        if exploring[0]:
            ...
            if paused[0]:
                ...
                ax_explore.set_title('已暂停', color="#e80606")
            else:
                btn_pause.label.set_text('暂停')
                ax_explore.set_title('正在探索...', color="#122bea")
            print("暂停" if paused[0] else "继续")
            fig.canvas.draw_idle()
    ...
    # ========== 按钮UI布局 ==========
    fig, (ax_full, ax_explore) = plt.subplots(1, 2, figsize=(14, 7))
    fig.canvas.manager.set_window_title('雷达小车SLAM探索地图')
    plt.subplots_adjust(bottom=0.15, wspace=0.3)
    ax_start = plt.axes([0.15, 0.02, 0.09, 0.045])
    ax_pause = plt.axes([0.41, 0.02, 0.09, 0.045])
    ax_back = plt.axes([0.9, 0.94, 0.065, 0.045])
    btn_start = Button(ax_start, '开始探索', color='#4aa3f7', hovercolor='#e6e8ea')
    btn_pause = Button(ax_pause, '暂停', color='#4aa3f7', hovercolor='#e6e8ea')
    btn_back = Button(ax_back, '退出SLAM扫描', color="#020942", hovercolor="#696fbbad")
    ...
    while True:
        # 等待"开始探索"按钮
        while not exploring[0] and plt.fignum_exists(fig.number):
            plt.pause(0.1)
        ...
        if len(sys.argv) > 1:
            ...
            map_file = 'maze_edges.json'
            print(f"未提供迷宫文件参数，使用默认文件: {map_file}")
            
        # 加载地图文件以获取墙体线段
        load_full_map(map_file)
        print(f"已加载地图文件，包含 {len(map_segments)} 条墙体线段")
        ...
        # 只需在探索过程中加暂停控制
        visited = set()
        car_pos[0], car_pos[1] = start_x, start_y
        vehicle_controller.robot.x = start_x
        vehicle_controller.robot.y = start_y
        radar = get_radar()
        direction_checks = [
            (0, list(range(0, 180))),
            (1, list(range(270, 360)) + list(range(0, 90))),
            (2, list(range(180, 360))),
            (3, list(range(90, 270)))
        ]
        for d, indices in direction_checks:
            far_count = sum(1 for idx in indices if radar[idx] is not None and radar[idx] > 30)
        ...
        # ====== DFS探索，带暂停控制 ======
        def dfs_search_with_pause(pos, visited, axs):
            global exit_pos_list
            radar = get_radar()
            if sum(1 for d in radar if d > 40) >= 180:
                print("检测到出口，已走出迷宫边界，直接返回。")
                exit_pos_list.append(pos.copy())
                return
            key = tuple(pos)
            if key in visited:
                print("?????")
            ...
            visited.add(key)
            if axs is not None:
                update_plot(axs[0], axs[1])
                plt.pause(0.00000001)
            directions = [(0, 1), (1, 0), (0, -1), (-1, 0)]
            for d, (dx, dy) in enumerate(directions):
                next_pos = [pos[0] + dx * 1.0, pos[1] + dy * 1.0]
                next_key = tuple(next_pos)
                if next_key in visited:
                    print("visited!")
                    continue
                if can_move_dfs(d):
                    ...
                    for _ in range(steps):
                        # ====== 暂停控制 ======
                        while exploring[0] and paused[0]:
                            plt.pause(0.1)
                        if not exploring[0]:
                            return
                        global isnext
                    ...
                    if axs is not None:
                        ...
                        plt.pause(0.00000001)
                    _ = get_radar()
                    dfs_search_with_pause(car_pos.copy(), visited, axs)
                    for _ in range(steps):
                        while exploring[0] and paused[0]:
                            plt.pause(0.1)
                        if not exploring[0]:
        ...
        dfs_search_with_pause(car_pos.copy(), visited, [ax_explore, ax_full])

        # ====== 统计出口和点云数据，供主循环外使用 ======
        # 筛选有效出口位置
        valid_exits = []
        for pos in exit_pos_list:
            dist = np.hypot(pos[0] - start_x, pos[1] - start_y)
            if dist >= 1:
                valid_exits.append(pos)
        # 合并距离小于1.1m的出口点
        merged_exits = valid_exits.copy()
            ...
            while i < len(merged_exits):
                ...
                while j < len(merged_exits):
                    ...
                    if dist < 1.1:
                        ...
                        merged_exits.pop(j)
                        merged_exits.pop(i)
                        merged_exits.append([mid_x, mid_y])
                        merged = True
                        break
                    j += 1
                if merged:
        ...
        # Exploration completed successfully, save the results
        print("\n=== Exploration Complete ===")
        # Save both result files
        pkl_file, txt_file = save_exploration_results(map_file, history_points_cleaned, start_x, start_y, merged_exits)
        print(f"\nFiles saved: \n- {pkl_file} (exploration results)\n- {txt_file} (radar data)")
        
        # Show the final visualization
        show_exploration_results(history_points_cleaned, start_x, start_y, merged_exits)
        break  # Exit the loop
